...
CREATE UNIQUE INDEX IF NOT EXISTS idx_payments_return_id_unique
ON public.payments(return_id)
...
BEGIN
  ...
  IF NOT FOUND THEN
    RAISE EXCEPTION 'রিটার্ন পাওয়া যায়নি';
  END IF;
  ...
  IF ret.is_audit_only = false AND ret.stock_applied = false THEN
    SELECT stock_quantity, imei INTO prod_row
    ...
    IF FOUND THEN
      ...
      SET stock_quantity = CASE
            ...
            ELSE COALESCE(prod_row.stock_quantity, 0) + ret.quantity
          END,
          updated_at = now()
      WHERE id = ret.product_id;
    END IF;
    ...
    IF ret.refund_method = 'exchange' AND ret.exchange_product_id IS NOT NULL AND ret.exchange_quantity > 0 THEN
      SELECT stock_quantity INTO exchange_row
      FROM public.products
  ...
  IF ret.is_audit_only = false AND ret.finance_applied = false THEN
    ...
    IF FOUND THEN
      exchange_value := CASE WHEN ret.refund_method = 'exchange'
        THEN COALESCE(ret.exchange_unit_price, 0) * COALESCE(ret.exchange_quantity, 0)
        ELSE 0
      END;
      ...
      SET total_amount = new_total,
          ...
          due_amount = new_due,
          status = CASE WHEN new_total = 0 THEN 'returned' ELSE 'completed' END,
          updated_at = now()
      WHERE id = ret.sale_id;

      IF ret.refund_method = 'cash' AND cash_refund > 0 AND ret.customer_id IS NOT NULL THEN
        INSERT INTO public.payments (
          sale_id, customer_id, amount, payment_method, notes, collected_by, return_id
        ) VALUES (
          ret.sale_id,
          ret.customer_id,
          -cash_refund,
          'cash',
          'সেলস রিটার্ন রিফান্ড: ' || COALESCE(ret.return_number, ret.id::text),
          _actor_id,
          ret.id
        )
...
REVOKE ALL ON FUNCTION public.process_sales_return(uuid, text, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.process_sales_return(uuid, text, uuid, text) TO authenticated;