        ...
        <TabsContent value="returns" className="space-y-3 pb-6">
          ...
          {filteredReturns.map((ret: any) => (
            <Card key={ret.id} className="p-4 space-y-3">
              ...
              <div className="flex gap-2 justify-end flex-wrap"><Button size="sm" variant="outline" onClick={() => setDetailsReturn(ret)}><Eye className="h-4 w-4 mr-1" />বিস্তারিত</Button>{ret.status === "pending" && <Button size="sm" variant="outline" onClick={() => openEditDialog(ret)}><Edit className="h-4 w-4 mr-1" />এডিট</Button>}<Button size="sm" variant="outline" onClick={() => generateSupplierReturnReceiptPdf(ret)}><Download className="h-4 w-4 mr-1" />PDF</Button>{ret.status === "completed" && <Button size="sm" variant="outline" onClick={() => printReceipt(ret)}><Printer className="h-4 w-4 mr-1" />রসিদ পুনঃপ্রিন্ট</Button>}{ret.status === "pending" && canApprove && <><Button size="sm" onClick={() => approveMutation.mutate(ret)} disabled={approveMutation.isPending}><CheckCircle className="h-4 w-4 mr-1" />অনুমোদন</Button><Button size="sm" variant="destructive" onClick={() => setRejectingId(ret.id)}><XCircle className="h-4 w-4 mr-1" />প্রত্যাখ্যান</Button></>}</div>
            </Card>
          ))}
